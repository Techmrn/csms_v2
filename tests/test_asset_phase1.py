...
def setup_items():
    async def _setup():
        async with AsyncSessionLocal() as session:
            ...
            con_item = Item(code="TEST-CON", name="Test Consumable", category_id=cat_con.id, unit_id=unit.id, is_active=True)
            asset_item = Item(code="TEST-ASS", name="Test Asset", category_id=cat_asset.id, unit_id=unit.id, is_active=True)
...
@pytest.fixture(scope="module")
def auth_headers():
    response = client.post("/api/auth/token", data={"username": "dev_general_sk", "password": "DevOnly123!"})
    assert response.status_code == 200
    return {"Authorization": f"Bearer {response.json()['access_token']}"}

def test_unauthenticated_asset_register():
    response = client.get("/api/assets")
    assert response.status_code == 401

def test_wrong_store_authenticated_user():
    response = client.post("/api/auth/token", data={"username": "dev_press_sk", "password": "DevOnly123!"})
    headers = {"Authorization": f"Bearer {response.json()['access_token']}"}
    
    response = client.get("/api/assets?store_id=1", headers=headers)
    assert response.status_code == 403

def test_asset_receipt_workflow(setup_items, auth_headers):
    ...
    # Verify receipt
    resp = client.post(f"/api/receipts/{receipt_id}/verify", headers=auth_headers)
    assert resp.status_code == 200, resp.text
    
    # Post receipt
    resp = client.post(f"/api/receipts/{receipt_id}/post", headers=auth_headers)
    assert resp.status_code == 200, resp.text
    ...
    async def check_assets():
        async with AsyncSessionLocal() as session:
            ...
            assert len(assets) == 2
            assert any(a.serial_no == "SN-001" for a in assets)
