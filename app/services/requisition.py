...
class RequisitionService:
    ...
    async def create(self, payload: RequisitionCreate):
        await self.auth.require_permission(payload.actor_id, "REQUISITION_CREATE")
    ...
    async def approve_branch(self, requisition_id: int, payload: BranchApprovalRequest, approve: bool = True):
        user = await self.auth.require_permission(payload.actor_id, "REQUISITION_APPROVE_BRANCH")
    ...
    async def approve_central(self, requisition_id: int, payload: CentralApprovalRequest):
        user = await self.auth.require_permission(payload.actor_id, "REQUISITION_APPROVE_CENTRAL")
        requisition = await self.repository.get(requisition_id, for_update=True)
        if requisition is None:
            raise HTTPException(404, "Requisition not found")
        ...
        if requisition.created_by == payload.actor_id or requisition.branch_approved_by == payload.actor_id:
        ...
        requisition.central_approved_by = payload.actor_id
    ...
    async def reject_branch(self, requisition_id: int, payload: BranchApprovalRequest):
        return await self.approve_branch(requisition_id, payload, approve=False)
